export const Scripts: ModdedBattleScriptsData = {
	...
	actions: {
        // 【关键修复】显式声明 this: any 和所有参数为 any，彻底屏蔽参数不足和上下文报错
        runMove(this: any, moveOrMoveName: any, pokemon: any, targetLoc: any, sourceEffect?: any, zMove?: any, externalMove?: any, maxMove?: any, originalTarget?: any) {
            
            const move = this.dex.getActiveMove(moveOrMoveName);
            const target = this.battle.getTarget(pokemon, maxMove || zMove || move, targetLoc, originalTarget);
            
            if (move.id === 'judgment' && pokemon.hasItem('legendplate') && pokemon.species.name === 'Arceus-Legend-Fantasy' && target) {
                
                let bestType = 'Normal';
                let maxEff = -4;
                let candidateTypes: string[] = [];

                for (const type of this.battle.dex.types.names()) {
                    if (type === '???' || type === 'Stellar') continue;
                    if (!this.battle.dex.getImmunity(type, target)) continue;
                    ...
                    if (immunities[type] && immunities[type].includes(targetAbility)) {
                        if (type === 'Ground' && (target.volatiles['smackdown'] || this.battle.field.getPseudoWeather('gravity'))) {
                            // 落地状态有效
                        } else {
                            continue; 
                        }
                    ...
                    for (const targetType of target.getTypes()) {
                        let e = this.battle.dex.getEffectiveness(type, targetType);
                        if (weather === 'deltastream' && targetType === 'Flying' && e > 0) e = 0; 
                        eff += e;
                    }
                ...
                if (candidateTypes.length > 0) {
                    bestType = this.battle.sample(candidateTypes);
                }

                if (pokemon.getTypes().join() !== bestType) {
                    pokemon.setType(bestType);
                    this.battle.add('-legendplate', pokemon, bestType);
                }
                
                move.type = bestType;
            }
            
            // 【关键修复】使用 call 绑定 this 继续执行原方法
            return super.runMove(moveOrMoveName, pokemon, targetLoc, sourceEffect, zMove, externalMove, maxMove, originalTarget);
        },
		// ==========================================
		// 1. 气场爆发 / 究极爆发的按钮判定逻辑
		// ==========================================
