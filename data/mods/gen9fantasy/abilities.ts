export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable = {
	...
	beastboost: {
		onSourceAfterFaint(length, target, source, effect) {
			// 【关键】检查是否拥有 suppressability 状态，如果有，则不执行提升
			if (source.volatiles['suppressability']) return;
	...
	qiyizhizaozhe: {
        ...
        rating: 5,
        num: 10035,
        shortDesc: "登场引发重力与携带的空间;若携带戏法空间,首回合成功使出超能系招式,该招式未被封印,回合末将其制造",
    },
	yanbuzhen: {
		onDamagingHit(damage, target, source, move) {
			// 确定对手的场地侧
	...
	leizhu: {
        onStart(pokemon) {
            ...
            // 新增：附加能力提升专用的状态，这会将计算逻辑转移到 conditions.ts 中
            pokemon.addVolatile('leizhustats'); 
        },
        ...
        condition: {
            ...
            onAnyFaint(target, source, effect) {
                ...
                // 重点：如果存在旧的层数，先向客户端发送 -end 消除旧层数的 UI 标签
                if ((abilityHolder as any).leizhuStacks > 0) {
                    this.add('-end', abilityHolder, `leizhustack${(abilityHolder as any).leizhuStacks}`, '[silent]');
                }
                
                // 增加层数
                (abilityHolder as any).leizhuStacks++;
                
                // 重点：向客户端发送 -start，触发 UI 生成新的累积层数状态标签（类似大将的 Fallen 层数）
                this.add('-start', abilityHolder, `leizhustack${(abilityHolder as any).leizhuStacks}`, '[silent]');
                
                this.add('-message', `由于 ${target.name} 倒下，擂主 ${abilityHolder.name} 的双攻及双防永久提升了 10%！`);
            },
        },
        flags: {},
        name: "Lei Zhu",
