...
auraburstatk: {
    ...
    auraburstall: {
        ...
        onModifySpA(spa, pokemon) { return this.chainModify(1.2); },
        onModifySpD(spd, pokemon) { return this.chainModify(1.2); },
	...
	// =================================================================
	wenliznormal: { name: 'wenliznormal', noCopy: true, onStart(target) { this.add('-start', target, 'wenliznormal', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Normal'; } },
	wenlizfire: { name: 'wenlizfire', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizfire', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Fire'; } },
	wenlizwater: { name: 'wenlizwater', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizwater', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Water'; } },
	wenlizelectric: { name: 'wenlizelectric', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizelectric', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Electric'; } },
	wenlizgrass: { name: 'wenlizgrass', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizgrass', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Grass'; } },
	wenlizice: { name: 'wenlizice', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizice', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Ice'; } },
	wenlizfighting: { name: 'wenlizfighting', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizfighting', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Fighting'; } },
	wenlizpoison: { name: 'wenlizpoison', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizpoison', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Poison'; } },
	wenlizground: { name: 'wenlizground', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizground', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Ground'; } },
	wenlizflying: { name: 'wenlizflying', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizflying', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Flying'; } },
	wenlizpsychic: { name: 'wenlizpsychic', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizpsychic', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Psychic'; } },
	wenlizbug: { name: 'wenlizbug', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizbug', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Bug'; } },
	wenlizrock: { name: 'wenlizrock', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizrock', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Rock'; } },
	wenlizghost: { name: 'wenlizghost', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizghost', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Ghost'; } },
	wenlizdragon: { name: 'wenlizdragon', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizdragon', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Dragon'; } },
	wenlizdark: { name: 'wenlizdark', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizdark', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Dark'; } },
	wenlizsteel: { name: 'wenlizsteel', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizsteel', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Steel'; } },
	wenlizfairy: { name: 'wenlizfairy', noCopy: true, onStart(target) { this.add('-start', target, 'wenlizfairy', '[silent]'); }, onModifyTypePriority: -1, onModifyType(move, pokemon) { if (pokemon.moveSlots[0] && move.id === pokemon.moveSlots[0].id) move.type = 'Fairy'; } },
	emengchanrao: {
	...
	frz: {
        ...
        onBeforeMove(pokemon, target, move) {
            // --- 幻之冰之石的修正：在这里直接放行 ---
            if (pokemon.hasItem('fantasyicestone')) {
                return true; 
            }
