	...
	},
	{
		...
		onValidateSet(set) {
			...
			if (set.item && item.megaStone) {
				const megaSpecies = this.dex.species.get(typeof item.megaStone === 'string' ? item.megaStone : (Array.isArray(item.megaStone) ? item.megaStone[0] : Object.values(item.megaStone || {})[0]));
