...
战败免费无限重试整层，回退入层的队伍、经验、招式、进化、背包和金币，仅 Boss 层完成发 1 成长点（200 层共 23 点），其他层不发点。捕捉即时永久入账，同一冒险／楼层／路线／小场的对象跨重试去重。SQLite 事务保存请求凭据和状态，重复结算不重复奖励。满队安置和学招选择也可刷新续办。
...
- `moveMemory` 保存每个已解锁招式的剩余／最大 PP，更换技能不回满。旧存档已遗忘但无 PP 记录的技能保守置 0，休整恢复后可用；当前招式 PP 不变。中心仅恢复存活成员，包括未装备的已学招式。
...
- 对战 EV 取固定 PokéAPI 物种产出，Fantasy 形态缺少独立条目时回退基础物种。经验、EV、招式记忆、事件许可及队伍调整随入层快照一起回退。
- 招式详情使用 `Dex.mod("gen9fantasy")` 的名称、属性、分类、威力、命中、PP 与简介，配现有中文翻译和 PS 图标。
