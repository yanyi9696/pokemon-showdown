 ...
 * or a strength benchmark, and does not enforce the future five-second timeout.
 */
	...
	try {
		...
		for (let iteration = 0; !battle.ended && iteration < maxTurns * 8 + 10; iteration++) {
			...
			const pending = battle.sides.map((side, index) => {
				...
				const decision = policies[index].decide(observation, rng.getSeed(), rejected[index].get(fingerprint));
				rng.random();
